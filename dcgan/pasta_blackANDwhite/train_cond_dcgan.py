...
niter_decay = 5 # # of iter to linearly decay learning rate to zero
'''
...
#trX_vis = np.asarray([[trX[i] for i in py_rng.sample(tr_idxs[trY==y], 20)] for y in range(ny)]).reshape(200, -1)
#trX_vis = inverse_transform(transform(trX_vis))

trX_vis = np.asarray(trX).reshape(len(trX),-1)
trX_vis = inverse_transform(transform(trX_vis))
grayscale_grid_vis(trX_vis, (ny, 20), '../../../../opt/ADL_db/Users/sjain/dcgan/PastaBW/condgan/samples/%s_etl_test.png'%desc)

sample_zmb = floatX(np_rng.uniform(-1., 1., size=(20*ny, nz)))
sample_ymb = floatX(OneHot(np.asarray([[i for _ in range(20)] for i in range(ny)]).flatten(), ny))

def gen_samples(n, nbatch=nbatch):
    samples = []
    labels = []
    n_gen = 0
...
for epoch in range(1, niter+niter_decay+1):
    ...
    grayscale_grid_vis(inverse_transform(samples), (ny, 20), '../../../../opt/ADL_db/Users/sjain/dcgan/PastaBW/condgan/samples/%s/%d.png'%(desc, n_epochs))
    n_epochs += 1
    if n_epochs > niter:
        lrt.set_value(floatX(lrt.get_value() - lr/niter_decay))
    if n_epochs in [500,1000,1500,2000,2500,3000,3500,3999,4500,5000,5500,5999,6500,6999,7500,7999,8500,8999]:
        joblib.dump([p.get_value() for p in gen_params],  '../../../../opt/ADL_db/Users/sjain/dcgan/PastaBW/condgan/models/%s/%d_gen_params.jl'%(desc, n_epochs))
        joblib.dump([p.get_value() for p in discrim_params], '../../../../opt/ADL_db/Users/sjain/dcgan/PastaBW/condgan/models/%s/%d_discrim_params.jl'%(desc, n_epochs))