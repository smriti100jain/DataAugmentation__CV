...
categorical_list = [52] # Each entry in this list defines a categorical variable of a specific size.
...
sample_directory = '../../../opt/ADL_db/Users/sjain/Infogan/Cars/figsTut' #Directory to save sample images from generator in.
model_directory = '../../../opt/ADL_db/Users/sjain/Infogan/Cars/models' #Directory to save trained model to.
...
with tf.Session() as sess:  
    ...
    for i in range(iterations):
        ...
        if i % 100 == 0:
            ...
            z_sample = np.random.uniform(-1.0,1.0,size=[520,z_size]).astype(np.float32) #Generate another z batch
            lcat_sample = np.reshape(np.array([e for e in range(52) for _ in range(10)]),[520,1])
            a = a = np.reshape(np.array([[(e/4.5 - 1.)] for e in range(52) for _ in range(10)]),[52,10]).T
            b = np.reshape(a,[520,1])
            c = np.zeros_like(b)
            ...
            save_images(np.reshape(samples[0:520],[520,img_row,img_col]),[52,10],sample_directory+'/fig'+str(i)+'.png')
        if i % 1000 == 0 and i != 0:
