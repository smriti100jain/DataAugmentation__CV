...
sample_directory = '../../../opt/ADL_db/Users/sjain/InfoganSelf/Mnist/figsTut' #Directory to save sample images from generator in.
model_directory = '../../../opt/ADL_db/Users/sjain/InfoganSelf/Mnist/Models' #Directory to save trained model to.
...
for target in range(0,10):
            createFold = os.path.join(sample_directory,'Data',str(target))
            if not os.path.exists(createFold):
...
with tf.Session() as sess:  
    ...
    z_sample = np.random.uniform(-1.0,1.0,size=[10000,z_size]).astype(np.float32) #Generate another z batch
    lcat_sample = np.reshape(np.array([e for e in range(10) for _ in range(1000)]),[10000,1])
    a = a = np.reshape(np.array([[(e/4.5 - 1.)] for e in range(10) for _ in range(1000)]),[10,1000]).T
    b = np.reshape(a,[10000,1])
    c = np.zeros_like(b)
    lcont_sample = np.hstack([b,c])
    samples = sess.run(Gz,feed_dict={z_in:z_sample,latent_cat_in:lcat_sample,latent_cont_in:lcont_sample}) #Use new z to get sample images from generator.
    np.save(os.path.join(sample_directory,'x.pkl'),np.reshape(samples[0:10000],[10000,32,32]))
    np.save(os.path.join(sample_directory,'y.pkl'),lcat_sample)
    X = np.reshape(samples[0:10000],[10000,32,32])
    y = lcat_sample
    for numm in range(np.shape(X)[0]):
        image = X[numm]
        tar = int(y[numm])
        scipy.misc.imsave(os.path.join(sample_directory,'Data',str(tar),str(numm)+'.png'), image)
    '''
    if not os.path.exists(sample_directory):
        os.makedirs(sample_directory)
    #Save sample generator images for viewing training progress.
