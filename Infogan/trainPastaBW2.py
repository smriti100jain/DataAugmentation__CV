...
def discriminator(bottom, cat_list,conts, reuse=False):
    ...
    dis1 = slim.convolution2d(bottom,32*2,[3,3],padding="SAME",\
    ...
    print(1)
    print(dis1)
    dis2 = slim.convolution2d(dis1,64*2,[3,3],padding="SAME",\
        normalizer_fn=slim.batch_norm,activation_fn=lrelu,\
        reuse=reuse,scope='d_conv2', weights_initializer=initializer)
    dis2 = tf.space_to_depth(dis2,2)
    print(dis2)
    dis3 = slim.convolution2d(dis2,128*2,[3,3],padding="SAME",\
        normalizer_fn=slim.batch_norm,activation_fn=lrelu,\
        reuse=reuse,scope='d_conv3',weights_initializer=initializer)
    dis3 = tf.space_to_depth(dis3,2)
    print(dis3)
    dis4 = slim.fully_connected(slim.flatten(dis3),1024*2,activation_fn=lrelu,\
    ...
    print(d_out)
    q_a = slim.fully_connected(dis4,128*2,normalizer_fn=slim.batch_norm,\
...
categorical_list = [13] # Each entry in this list defines a categorical variable of a specific size.
...
real_in = tf.placeholder(shape=[None,img_row,img_col,1],dtype=tf.float32) #Real images
...
trainerD = tf.train.AdamOptimizer(learning_rate=0.000002,beta1=0.5)
trainerG = tf.train.AdamOptimizer(learning_rate=0.000002,beta1=0.5)
trainerQ = tf.train.AdamOptimizer(learning_rate=0.000002,beta1=0.5)
...
for classLabel in range(len(classes)):
        ...
        trainData =[scipy.misc.imresize(rgb2gray(scipy.ndimage.imread(classPath+'/'+classSamples[i],flatten=False)),size=(img_row,img_col)) for i in indices]
        if(classLabel==0):
            Images = trainData
        else:
            Images = np.vstack((Images,trainData))
...
batch_size = np.shape(Images)[0] #Size of image batch to apply at each iteration.
iterations = 500000#500000 #Total number of iterations to use.
sample_directory = '../../../opt/ADL_db/Users/sjain/Infogantemp/Pasta64/figsTut' #Directory to save sample images from generator in.
model_directory = '../../../opt/ADL_db/Users/sjain/Infogantemp/Pasta64/models' #Directory to save trained model to.
...
with tf.Session() as sess:  
    ...
    for i in range(iterations):
        ...
        xs = Images#mnist.train.next_batch(batch_size) #Draw a sample batch from MNIST dataset.
        xs = (np.reshape(xs,[batch_size,img_row,img_col,1]) - 0.5) * 2.0 #Transform it to be between -1 and 1
        #xs = np.reshape(xs,[batch_size,img_row,img_col,1])
        ...
        if i % 100 == 0:
            ...
            z_sample = np.random.uniform(-1.0,1.0,size=[130,z_size]).astype(np.float32) #Generate another z batch
            lcat_sample = np.reshape(np.array([e for e in range(13) for _ in range(10)]),[130,1])
            a = a = np.reshape(np.array([[(e/4.5 - 1.)] for e in range(13) for _ in range(10)]),[13,10]).T
            b = np.reshape(a,[130,1])
            c = np.zeros_like(b)
            ...
            save_images(np.reshape(samples[0:130],[130,img_row,img_col]),[13,10],sample_directory+'/fig'+str(i)+'.png')
        if i % 1000 == 0 and i != 0:
